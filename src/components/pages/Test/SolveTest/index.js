...
class SolveTest extends PureComponent {
  ...
  setClock = () => {
    ...
    if (diff <= 0) {
      ...
      this.clockRunTimeout = setTimeout(
        this.runClock,
        secondsUntil10mins * 1000
      );
    }
  };
  ...
  sendAnswer = () => {
    return new Promise(resolve => {
      ...
      const activeQuestionIndex = questions.findIndex(
        question => question._id === activeQuestion._id
      );
      ...
      axios
        .put(
          `tests/answer/${this.props.testId}/${questions[activeQuestionIndex]._id}`,
        ...
        .catch(err => {
          if (
            ...
            err.response.reason === "test_not_ongoing"
          ) {
            // Test is finished
            this.setState({ finished: true });
          } else {
            ...
            {finished ? (
              <div className="test-solve__info">
                <h2 className="text-error">
                  <Trans i18nKey="test.finished.header" />
                </h2>
                <p>
                  <Trans i18nKey="test.finished.description" />
                </p>
