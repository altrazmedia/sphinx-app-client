...
const List = props => {
  ...
  if (tests.length === 0) {
    return (
      <Illustration
        variant="empty"
        description={<Trans i18nKey={`tests.empty.${status}`} />}
      />
    );
  }
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            <th>
              <Trans i18nKey="test.start" />
            </th>
            <th>
              <Trans i18nKey="test.end" />
            </th>
            {tests[0].hasOwnProperty("questionsAnswered") && (
              <th>
                <Trans i18nKey="test.questionAnswered" />
              </th>
            )}
            <th>
        ...
        <tbody>
          {tests.map(test => (
            <tr key={test._id}>
              ...
              {test.hasOwnProperty("questionsAnswered") && (
                <td>
                  {test.questionsAnswered} / {test.questionsQuantity}
                </td>
              )}
              <td>
                <Button
