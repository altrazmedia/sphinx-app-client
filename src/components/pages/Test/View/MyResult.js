    ...
    <div className="my-result">
      <ColumnView>
        <ColumnView.Item 
          ...
          value={
            <span className={score >= 50 ? "text-success" : "text-error"}>{score} %</span>
          }
        />
        <ColumnView.Item 
        ...
        questions.map(question => (
          <div className="result-question" key={question._id}>
            <h3 className="result-question__header">
              <Icon name={question.correct ? "check-circle" : "times-circle"} color={question.correct ? "success" : "error"} />  
              {question.content}
            </h3>
            <p className="result-question__answers">
              <span className="text-light">{<Trans i18nKey="test.yourAnswer" />}: </span>
              {
                question.options
            ...
            <p className="result-question__answers">
              <span className="text-light">{<Trans i18nKey="test.correctAnswer" />}: </span>
              {
                question.options
