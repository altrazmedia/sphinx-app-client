...
import { Modal, ErrorMessage, Input, Select, Loader, Button } from "components/common";
...
class AddCourse extends PureComponent {
  ...
  render = () => {
    ...
    const fetchError = Boolean(users.error) || Boolean(subjects.error) || Boolean(groups.error); // Errors occurred while downloading data
        ...
        <Modal.Buttons>
          {
            loading ? null 
            : fetchError ? 
              <Button variant="text" onClick={close}>{t("cancel")}</Button>
            : <>
                <Button variant="text" onClick={close}>{t("cancel")}</Button>
                <Button type="submit" form={FORM_ID} >{t("save")}</Button>
              </>
          }
        </Modal.Buttons>
      </Modal>
...
export default connect(READ, EMIT)(withTranslation()(AddCourse))